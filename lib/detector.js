...
class Detector {
    ...
    saveTarget(data, options) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (opts.hasOwnProperty('withRelated') && opts.withRelated.indexOf(key) !== -1 && existingRelation && existingRelation.id) {
                opts.method = 'update';
    ...
    getMatchingTarget(data, options) {
        let opts = _.cloneDeep(options);
        let properties = _.cloneDeep(data.properties);
            ...
            .then((matching) => {
                if (foreignKeyLookup && !matching) {
                    throw new errors.BookshelfRelationsError({
                        ...
                        context: JSON.stringify(data.properties)
                    });
                }
    ...
    saveTargets(data, options) {
        ...
        return Promise.each(newTargets, (properties) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                if (properties.hasOwnProperty('id') && Object.keys(properties).length === 1) {
                    results.push(result);
                    return;
                }
