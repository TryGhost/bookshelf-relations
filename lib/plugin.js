...
module.exports = function relationsPlugin(bookshelf, pluginOptions) {
    ...
    let Model = bookshelf.Model.extend({
        initialize: function initialize() {
            ...
            // CASE: These are the parent events, register specific events after bookshelf-relations
            // Reason: The plugin first need to update the relations, before the parent can receive the events
            // Otherwise you can run into missing data.
            let clonedEvents = _.cloneDeep(this._events);

            if (this._events.created) {
                this.off('created', this._events.created);
            }

            if (this._events.updated) {
                this.off('updated', this._events.updated);
            }

            if (this._events.saved) {
                this.off('saved', this._events.saved);
            }
            ...
            if (clonedEvents.created) {
                this.on('created', clonedEvents.created);
            }

            if (clonedEvents.updated) {
                this.on('updated', clonedEvents.updated);
            }

            if (clonedEvents.saved) {
                this.on('saved', clonedEvents.saved);
            }
