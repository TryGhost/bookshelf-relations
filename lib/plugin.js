...
module.exports = function relationsPlugin(bookshelf, pluginOptions) {
    ...
    let Model = bookshelf.Model.extend({
        initialize: function initialize() {
            /**
             * @NOTE:
             * For the relation "belongsTo", we have to insert the relation **BEFORE** adding
             * the resource, because the resource has a reference on the table.
             * Otherwise it will fail with a database error.
             */
            ...
            this.on('created', (model, attributes, options) => {
                ...
                this.relationships.forEach((key) => {
                    if (this['relationships_' + key]) {
                        ...
                        if (type === 'belongsTo' && options.method === 'insert') {
                            return;
                        }
