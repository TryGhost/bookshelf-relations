        ...
        if (model.get(key) || options.event === 'destroying') {
            ...
            if (options.event === 'destroying') {
                ...
                if (relationshipConfig.editable === true) {
                    ...
                    if (Object.keys(relationValue).filter(k => (k !== 'id')).length) {
                        debug('Stripping relation from extra values: ', Object.keys(relationValue).filter(k => (k !== 'id')));
                    }
                    if (relationValue?.id) {
                        relations[type][key] = {id: relationValue.id};
                    }
                }
            }
        }
