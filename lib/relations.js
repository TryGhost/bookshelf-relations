...
class Relations {
                ...
                return setter
                    ...
                    .catch((err) => {
                        if (
                            ...
                            (_.isArray(err) && errors.utils.isGhostError(err[0]))
                        ) {
                            throw err;
                        }
    ...
    setBelongsTo(data, options) {
        ...
        if (opts.method === 'insert') {
            return this.detector.saveTarget(targetData, opts).then(function (result) {
            ...
            return this.detector
                ...
                .then((result) => {
                    ...
                    return toSave.save(null, opts).then(() => {
                        return result;
                    });
                })
                .then((result) => {
    ...
    setHasOne(data, options) {
        ...
        if (
            ...
            !_.isEmpty(targetData.properties) &&
            !targetData.properties.id
        ) {
            return this.detector
                .getExistingTargets({ model: model, key: key }, opts)
                .then((existingTargets) => {
                    ...
                    return this.detector.saveTarget(targetData, opts).then((result) => {
                        model.relations[key] = result;
    ...
    setBelongsToMany(data, options) {
        ...
        return this.detector
            .getExistingTargets({ model: model, key: key, pluginOptions: pluginOptions }, opts)
            ...
            .then((targets) => {
                ...
                function getHook(name, fallbackName) {
                    const globalHook = getFn(
                        pluginOptions?.hooks?.belongsToMany,
                        name,
                        fallbackName,
                    );
                ...
                return Promise.resolve()
                    .then(function () {
                        ...
                        return existingTargets
                            .attach(targetsToAttach, _.pick(opts, ['transacting']))
                            .then(function () {
                                if (!changed[key]) {
                                    changed[key] = {
                                        attached: [],
